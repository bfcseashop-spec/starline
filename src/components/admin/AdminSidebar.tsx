import { Link } from "react-router-dom";
import {
  LayoutDashboard, Home, Users, FileText, BarChart3, Settings, 
  CreditCard, FolderOpen, Images, ClipboardList, LogOut, ChevronLeft, ChevronRight, Sparkles,
  Shield, Landmark, Share2, Wallet, PanelTop, Navigation, TrendingUp, Menu, X,
} from "lucide-react";
import { useState } from "react";
import { useAuth } from "@/hooks/useAuth";

type AdminPage =
  | "dashboard" | "customers" | "projects" | "payments" | "images"
  | "work_updates" | "documents" | "settings"
  | "roles" | "invoices" | "social_media" | "bank_statement" | "reports" | "expenses"
  | "hero_section" | "header_management" | "investment";

type NavItem = { id: AdminPage; icon: typeof LayoutDashboard; label: string; gradient: string };
type NavSection = { title: string; items: NavItem[] };

const navSections: NavSection[] = [
  {
    title: "Main",
    items: [
      { id: "dashboard", icon: LayoutDashboard, label: "Dashboard", gradient: "bg-dash-blue" },
      { id: "customers", icon: Users, label: "Customers", gradient: "bg-dash-green" },
      { id: "projects", icon: Home, label: "Properties", gradient: "bg-dash-orange" },
    ],
  },
  {
    title: "Finance",
    items: [
      { id: "payments", icon: CreditCard, label: "Payments", gradient: "bg-dash-teal" },
      { id: "invoices", icon: FileText, label: "Create Invoice", gradient: "bg-dash-purple" },
      { id: "expenses", icon: Wallet, label: "Expenses", gradient: "bg-dash-orange" },
      { id: "investment", icon: TrendingUp, label: "Investment", gradient: "bg-dash-green" },
      { id: "bank_statement", icon: Landmark, label: "Bank Statement", gradient: "bg-dash-teal" },
      { id: "reports", icon: BarChart3, label: "Reports", gradient: "bg-dash-purple" },
    ],
  },
  {
    title: "Content",
    items: [
      { id: "images", icon: Images, label: "Property Images", gradient: "bg-dash-purple" },
      { id: "work_updates", icon: ClipboardList, label: "Work Updates", gradient: "bg-dash-teal" },
      { id: "documents", icon: FolderOpen, label: "Documents", gradient: "bg-dash-pink" },
      { id: "social_media", icon: Share2, label: "Social Media", gradient: "bg-dash-blue" },
    ],
  },
  {
    title: "System",
    items: [
      { id: "hero_section", icon: PanelTop, label: "Hero Section", gradient: "bg-dash-orange" },
      { id: "header_management", icon: Navigation, label: "Header", gradient: "bg-dash-green" },
      { id: "roles", icon: Shield, label: "Roles", gradient: "bg-dash-pink" },
      { id: "settings", icon: Settings, label: "Settings", gradient: "bg-dash-teal" },
    ],
  },
];

interface AdminSidebarProps {
  activePage: AdminPage;
  onPageChange: (page: AdminPage) => void;
}

const SidebarContent = ({ activePage, onPageChange, collapsed, user, signOut, onItemClick }: {
  activePage: AdminPage;
  onPageChange: (page: AdminPage) => void;
  collapsed: boolean;
  user: any;
  signOut: () => void;
  onItemClick?: () => void;
}) => (
  <>
    <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-4">
      {navSections.map((section) => (
        <div key={section.title}>
          {!collapsed && (
            <p className="px-3 mb-1.5 text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60">
              {section.title}
            </p>
          )}
          {collapsed && <div className="border-t border-border mx-3 mb-2" />}
          <div className="space-y-0.5">
            {section.items.map((item) => {
              const active = activePage === item.id;
              return (
                <button
                  key={item.id}
                  onClick={() => { onPageChange(item.id); onItemClick?.(); }}
                  title={collapsed ? item.label : undefined}
                  className={`flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-semibold transition-all duration-200 w-full text-left ${
                    active
                      ? "bg-muted text-foreground shadow-sm"
                      : "text-foreground/70 hover:text-foreground hover:bg-muted/50"
                  }`}
                >
                  <div className={`p-1.5 rounded-lg shrink-0 ${item.gradient}`}>
                    <item.icon size={15} className="text-white" />
                  </div>
                  {!collapsed && <span className="truncate">{item.label}</span>}
                </button>
              );
            })}
          </div>
        </div>
      ))}
    </nav>

    <div className="px-2 pb-4 shrink-0 space-y-2">
      {!collapsed && (
        <div className="bg-muted/60 rounded-xl p-3">
          <p className="text-[11px] text-muted-foreground truncate">Admin</p>
          <p className="text-xs text-foreground truncate font-medium">{user?.email}</p>
        </div>
      )}
      <button
        onClick={signOut}
        className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all w-full"
      >
        <LogOut size={16} className="shrink-0" />
        {!collapsed && <span>Sign Out</span>}
      </button>
    </div>
  </>
);

const AdminSidebar = ({ activePage, onPageChange }: AdminSidebarProps) => {
  const [collapsed, setCollapsed] = useState(false);
  const [mobileOpen, setMobileOpen] = useState(false);
  const { signOut, user } = useAuth();

  return (
    <>
      {/* Mobile menu button - rendered in header via portal-like approach */}
      <button
        onClick={() => setMobileOpen(true)}
        className="lg:hidden fixed top-4 left-4 z-50 p-2 rounded-xl bg-card border border-border shadow-md text-foreground"
        aria-label="Open menu"
      >
        <Menu size={20} />
      </button>

      {/* Mobile overlay sidebar */}
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50">
          <div className="absolute inset-0 bg-background/80 backdrop-blur-sm" onClick={() => setMobileOpen(false)} />
          <aside className="absolute left-0 top-0 bottom-0 w-64 bg-card border-r border-border flex flex-col shadow-2xl animate-in slide-in-from-left duration-200">
            <div className="px-4 h-16 flex items-center gap-3 border-b border-border shrink-0">
              <div className="w-9 h-9 rounded-xl bg-gold-gradient flex items-center justify-center shrink-0">
                <Sparkles size={18} className="text-accent-foreground" />
              </div>
              <Link to="/" className="font-heading text-base font-bold text-foreground truncate">
                Starline<span className="text-gold"> Ltd.</span>
              </Link>
              <button onClick={() => setMobileOpen(false)} className="text-muted-foreground hover:text-foreground ml-auto">
                <X size={18} />
              </button>
            </div>
            <SidebarContent activePage={activePage} onPageChange={onPageChange} collapsed={false} user={user} signOut={signOut} onItemClick={() => setMobileOpen(false)} />
          </aside>
        </div>
      )}

      {/* Desktop sidebar */}
      <aside
        className={`hidden lg:flex bg-card border-r border-border flex-col h-screen sticky top-0 transition-all duration-300 ${
          collapsed ? "w-[68px]" : "w-60"
        }`}
      >
        <div className="px-4 h-16 flex items-center gap-3 border-b border-border shrink-0">
          <div className="w-9 h-9 rounded-xl bg-gold-gradient flex items-center justify-center shrink-0">
            <Sparkles size={18} className="text-accent-foreground" />
          </div>
          {!collapsed && (
            <Link to="/" className="font-heading text-base font-bold text-foreground truncate">
              Starline<span className="text-gold"> Ltd.</span>
            </Link>
          )}
          <button onClick={() => setCollapsed(!collapsed)} className="text-muted-foreground hover:text-foreground transition-colors ml-auto">
            {collapsed ? <ChevronRight size={16} /> : <ChevronLeft size={16} />}
          </button>
        </div>
        <SidebarContent activePage={activePage} onPageChange={onPageChange} collapsed={collapsed} user={user} signOut={signOut} />
      </aside>
    </>
  );
};

export default AdminSidebar;
export type { AdminPage };
